...
class SVDpp {
    ...
    SVDpp(int latent_factors, int numThreads) {
    ...
    double train(Dataset data, int epochs, double lr, double reg) {
        ...
        int threadInc = (lf / numThreads) -1; // make sure beforehand that lf divisible by numThreads
        cout<<"threadInc"<<threadInc<<endl;
        ...
        for (int ep = 0; ep < epochs; ep++) {
            ...
            for (int i = 0; i < data.user_ids.size(); i++) {
                ...
                //for (int j = 0; j < lf; j += numThreads) {
                    //update(data, u, m, dr, j, lr, reg);
                    //thread th1(load_data, "../data/mu_train.csv"); 
                    //th1.join();

                    //updateSVDpp(u, m, dr, j, lr, reg, p, q, y, data.mpu); 

                    //int startsEnds[11] = {j, j + threadInc, j + (2 * threadInc), j + (3 * threadInc), j + (4 * threadInc), j + (5 * threadInc), \
                    //    j + (6 * threadInc), j + (7 * threadInc), j + (8 * threadInc), j + (9 * threadInc), j + (10 * threadInc)};

                    /*
                    thread th1(updateSVDpp, u, m, dr, startsEnds[0], startsEnds[1], lr, reg, p, q, y, ref(data.mpu));
                    thread th2(updateSVDpp, u, m, dr, startsEnds[1], startsEnds[2], lr, reg, p, q, y, ref(data.mpu));
                    thread th3(updateSVDpp, u, m, dr, startsEnds[2], startsEnds[3], lr, reg, p, q, y, ref(data.mpu));
                    thread th4(updateSVDpp, u, m, dr, startsEnds[3], startsEnds[4], lr, reg, p, q, y, ref(data.mpu));
                    thread th5(updateSVDpp, u, m, dr, startsEnds[4], startsEnds[5], lr, reg, p, q, y, ref(data.mpu));
                    thread th6(updateSVDpp, u, m, dr, startsEnds[5], startsEnds[6], lr, reg, p, q, y, ref(data.mpu));
                    thread th7(updateSVDpp, u, m, dr, startsEnds[6], startsEnds[7], lr, reg, p, q, y, ref(data.mpu));
                    thread th8(updateSVDpp, u, m, dr, startsEnds[7], startsEnds[8], lr, reg, p, q, y, ref(data.mpu));
                    thread th9(updateSVDpp, u, m, dr, startsEnds[8], startsEnds[9], lr, reg, p, q, y, ref(data.mpu));
                    thread th10(updateSVDpp, u, m, dr, startsEnds[9], startsEnds[10], lr, reg, p, q, y, ref(data.mpu));
                    */

                    thread th1(updateSVDpp, u, m, dr, 0, 10, lr, reg, p, q, y, ref(data.mpu));
                    thread th2(updateSVDpp, u, m, dr, 10, 20, lr, reg, p, q, y, ref(data.mpu));
                    thread th3(updateSVDpp, u, m, dr, 20, 30, lr, reg, p, q, y, ref(data.mpu));
                    thread th4(updateSVDpp, u, m, dr, 30, 40, lr, reg, p, q, y, ref(data.mpu));
                    thread th5(updateSVDpp, u, m, dr, 40, 50, lr, reg, p, q, y, ref(data.mpu));
                    thread th6(updateSVDpp, u, m, dr, 50, 60, lr, reg, p, q, y, ref(data.mpu));
                    thread th7(updateSVDpp, u, m, dr, 60, 70, lr, reg, p, q, y, ref(data.mpu));
                    thread th8(updateSVDpp, u, m, dr, 70, 80, lr, reg, p, q, y, ref(data.mpu));
                    thread th9(updateSVDpp, u, m, dr, 80, 90, lr, reg, p, q, y, ref(data.mpu));
                    thread th10(updateSVDpp, u, m, dr, 90, 100, lr, reg, p, q, y, ref(data.mpu));
                ...
                //}
                //if (i % 10000 == 0) {
                //    cout<<"round of latent factors done"<<ep<<" "<<i<<endl;
                //}
            }
        }
    }
